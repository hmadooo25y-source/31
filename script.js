...
function showScreen(targetKey) {
    ...
    if (screensWithLoading.includes(targetKey)) {
        ...
        if (currentScreen) {
            ...
            const delay = Math.floor(Math.random() * 2001) + 2000; // قللنا مدة التحميل قليلا
...
function executeScreenSwitch(targetKey) {
    ...
    if (screens[targetKey]) {
        ...
        // تم إزالة accountDetails من هنا لكي تأخذ display: flex بدلاً من block
        const blockScreens = ['login', 's1', 's4', 's6', 'notif'];
        targetScreen.style.display = blockScreens.includes(targetKey) ? 'block' : 'flex';

        // تم إزالة accountDetails من هنا لتثبيت الـ body والاعتماد على السكرول الداخلي في CSS
        ...
        if (contentDiv) {
            // التعديل الهام: إذا كانت شاشة تفاصيل الحساب، نجعلها flex ليتوافق مع الـ CSS الخاص بك
...
// دالة التحقق قبل الانتقال من الشاشة 8 إلى 9
function checkAndGoToS9() {
    const s8Input = document.getElementById('merchant-phone-input');
    if (!s8Input || s8Input.value.trim() === "") {
        return; // منع الانتقال بصمت
    }
    updateS9Inputs(); 
    showScreen('s9');
}
...
function addStatementEntry(descText, amountStr) {
    ...
    tr.innerHTML = `
        <td class="acc-col-date">${dateStr}</td>
...
document.querySelectorAll('.bank-item').forEach(item => {
    item.addEventListener('click', () => {
        ...
        // التحقق من أن المبلغ ورقم الموبايل ليسا فارغين أو 0
        if (val === 0 || phoneVal === "") {
            return; // إيقاف الدالة ومنع الانتقال بصمت
        }
...
if (nextBtnS9) {
    nextBtnS9.addEventListener('click', () => {
        ...
        // التحقق من أن المبلغ مدخل وليس 0 أو فارغ
        if (amount === "" || isNaN(numericAmount) || numericAmount === 0) {
            return; // يمنع فتح المودال والانتقال بصمت
        }
